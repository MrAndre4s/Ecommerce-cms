                    ...
                    <div class="card-body pt-0">
                        <table class="table align-middle table-row-dashed fs-6 gy-5">
                            ...
                            @foreach($deletedProductCategories as $productCategory)
                                <tr>
                                    ...
                                    <td class="text-end pe-0">
                                        <span class="fw-bold">
                                            {{ $productCategory->deleted_at->format('d/m/Y h:i') }}
                                        </span>
                                    </td>
                                    <td class="text-end pe-0">
                                    ...
                                    <td class="d-flex flex-row justify-content-end gap-2">
                                        <form action="{{ route('product-categories.restore', $productCategory) }}"
                                              method="post">
                                            @csrf
                                            <button type="submit" class="btn btn-primary">Восстановить</button>
                                        </form>
                                        <form action="{{ route('product-categories.force_delete', $productCategory) }}"
                                              method="post">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <div class="row">
                            <div class="col d-flex align-items-center justify-content-end justify-content-md-end">
                                {{ $deletedProductCategories->links() }}
                            </div>
                        </div>
                    </div>
                </div>
